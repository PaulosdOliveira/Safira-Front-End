    ...
    return (
        <div className="w-full flex flex-col min-h-screen">
            ...
            <main className="w-full  flex-1/2  bg-slate-100  py-20 sm:px-0 px-1">
                <FormLoginCandidato fecharModal={() => ""} goToMain={true} />
            </main>
            <Footer />
        </div>
    )
...
export const FormLoginCandidato: React.FC<formProps> = ({ goToMain, fecharModal }) => {

    const service = CandidatoService();
    const sessao = ServicoSessao();
    const router = useRouter();
    const { handleChange, handleSubmit, values } = useFormik<dadosLogin>({
